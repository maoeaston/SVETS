...
function safetyReportContent() {
  return {
    ...
    incident_snapshot: {
      ...
      context_phase: 'OFFLINE_ASSESSMENT',
...
function seedReport(paths) {
  ...
  const eventId = 'e2e-report-generated-event-001'
  ...
  const checksum = sha256Text(payloadJson)
...
VALUES
  (${sqlString(eventId)}, 'TASK_REPORT', ${sqlString(REPORT_ID)}, 'REPORT_GENERATED', 1,
   ${sqlString(payloadJson)}, ${sqlString(checksum)}, ${sqlString(actionLogPath)}, 2,
   ${sqlString(GENERATED_AT)}, 1, ${sqlString(GENERATED_AT)});
INSERT OR IGNORE INTO task_report
...
VALUES
  (${sqlString(REPORT_ID)}, 'SAFETY_TERMINATION_REPORT', ${sqlString(STUDENT_ID)}, 'SAFETY_INCIDENT',
   ...
   ${sqlString(eventId)}, ${sqlString(TEACHER_ID)}, ${sqlString(GENERATED_AT)},
   ...
   'safety-termination-report-v1.0', 'e2e-fixture', 'NORMAL', 'VALID', ${sqlString(eventId)}, 'GENERATED');
COMMIT;
`
  ...
  appendFileSync(actionLogPath, `${JSON.stringify(actionLogEntry)}\n`, { encoding: 'utf8' })
}
