...
const previousDeltaPaths = [2, 3, 4, 5, 6, 7, 8].map((step) => resolve(
  projectRoot,
  `scripts/fixtures/m5b-step${step}-source-delta-v1.json`
))
const deltaPath = resolve(projectRoot, 'scripts/fixtures/m5b-step9-source-delta-v1.json')
...
export function verifyM5bStep9SourceDelta() {
  ...
  for (let index = 0; index < deltas.length; index += 1) {
    ...
    if (deltas[index].step !== expectedStep || deltas[index].source_digest !== expectedSource) {
      throw new Error(`[m5b-step9-fixture] ${expectedStep} source chain is not accepted`)
    }
  }
  ...
  if (!exact) throw new Error('[m5b-step9-fixture] checkout does not match the reviewed M5B-9 source delta')
  ...
  if (existsSync(deltaPath)) {
    ...
    if (!exactJson(frozen, fixture)) {
      ...
      throw new Error('[m5b-step9-fixture] frozen source delta differs from checkout')
    }
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const delta = verifyM5bStep9SourceDelta()
  console.log(`[m5b-step9-fixture] source_delta=${delta.status}`)
  console.log(
    `[m5b-step9-fixture] digest=${delta.fixture.target_digest}, direct_added=${delta.fixture.added_target.length}, capability_added=${delta.fixture.capability_added_target.length}`
  )
}
