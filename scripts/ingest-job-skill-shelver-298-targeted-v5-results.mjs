...
const root = process.env.SVETS_V5_RESULTS_ROOT
  ? resolve(process.env.SVETS_V5_RESULTS_ROOT)
...
const paths = {
  ...
  previousGate: feature('job-skill-shelver-298-targeted-v4-rereview-merged-gate-v1.json'),
...
const expectedContentIds = [
  'M1_OP_038_V5', 'M2_OP_031_V5', 'M2_OP_038_V5',
  'M2_OP_041_V5', 'M5_DG_034_V5'
]
const expectedSafetyIds = ['M1_OP_037_V5', 'M1_OP_038_V5']
const expectedTracks = new Map([
  ['M1_OP_037_V5', ['SAFETY_TECHNICAL']],
  ['M1_OP_038_V5', ['CONTENT', 'SAFETY_TECHNICAL']],
  ['M2_OP_031_V5', ['CONTENT']],
  ['M2_OP_038_V5', ['CONTENT']],
  ['M2_OP_041_V5', ['CONTENT']],
  ['M5_DG_034_V5', ['CONTENT']]
])
...
function indexBy(records, label, selector = (record) => record.question_id) {
...
assert(hashRecord(candidateSetBody) === candidateSetHash, 'V5 candidate_set_hash drift')
assert(candidateSetHash === 'sha256:55b761ed6cf8b62460a79006d10ec37fa25a41702ab61b25fd26f185b8c403a7', 'Unexpected V5 candidate_set_hash')
assert(hashRecord(pendingGateBody) === pendingGateHash, 'V5 pending gate_hash drift')
assert(hashRecord(previousGateBody) === previousGateHash, 'V4 merged gate_hash drift')
assert(pendingGate.candidate_set?.candidate_set_hash === candidateSetHash, 'Pending gate and V5 candidate set do not match')
...
assert(safetySchema.properties?.reviewer?.const === '赫东', 'Safety result Schema reviewer drift')
assert(statSync(paths.content).size === 4769, 'Chen Xiaoqing reviewer export byte count changed')
assert(statSync(paths.safety).size === 1167, 'He Dong reviewer export byte count changed')
assert(hashFile(paths.content) === 'sha256:4577c7b88712dcd21ab7352651c5edb7f19e5cf9ce3fd01aa8e491768a25eab0', 'Chen Xiaoqing reviewer export bytes changed')
assert(hashFile(paths.safety) === 'sha256:27ccf836619c5f3008adf83fef32cc7e83ee8bf5b0551eedfb328134c80904aa', 'He Dong reviewer export bytes changed')

assert(candidates.summary?.source_total === 145, 'V5 source total must be 145')
assert(candidates.passed_question_references?.length === 139, 'V5 must retain exactly 139 passed references')
assertExactSet(candidates.questions.map((question) => question.question_id), expectedV5Ids, 'V5 candidate set')
assertExactSet(pendingGate.questions.map((question) => stripVersion(question.question_id)), pendingGate.questions.map((question) => stripVersion(question.question_id)), 'V5 pending gate logical question set')
assert(pendingGate.questions.length === 145, 'V5 pending gate must cover 145 questions')

const candidatesById = indexBy(candidates.questions, 'V5 candidates')
const previousById = indexBy(previousGate.questions, 'V4 merged questions')
const referencesById = indexBy(candidates.passed_question_references, 'V5 passed references')
assert(previousById.size === 145, 'V4 merged gate must cover 145 questions')
...
for (const candidate of candidates.questions) {
  ...
  assert(candidate.question_version === 5, `${candidate.question_id} question_version must be 5`)
  assert(candidate.previous_question_id === candidate.question_id.replace(/_V5$/, '_V4'), `${candidate.question_id} version chain mismatch`)
  assert(sameValues(candidate.required_review_tracks, expectedTracks.get(candidate.question_id)), `${candidate.question_id} failed review tracks drift`)
  const previous = previousById.get(candidate.previous_question_id)
  assert(previous?.status === 'RETURN_FOR_REVISION', `${candidate.question_id} previous V4 result was not returned`)
  ...
  assert(previous.next_question_id === candidate.question_id, `${candidate.question_id} does not match the V4 next_question_id`)
  assert(sameValues(previous.next_review_tracks, candidate.required_review_tracks), `${candidate.question_id} review track chain mismatch`)
}

for (const reference of candidates.passed_question_references) {
  const previous = previousById.get(reference.question_id)
  assert(previous, `${reference.question_id} passed reference is missing from the V4 merged gate`)
  assert(['PASSED', 'PASSED_V3_REFERENCE'].includes(previous.status), `${reference.question_id} is not a passed V4 reference`)
  assert(reference.question_version === previous.question_version, `${reference.question_id} referenced version drift`)
  assert(reference.semantic_hash === previous.new_semantic_hash, `${reference.question_id} referenced semantic hash drift`)
...
assert(new Set(logicalIds).size === 145, 'V5 references and candidates do not uniquely cover 145 logical questions')
assertExactSet(pendingGate.questions.map((question) => stripVersion(question.question_id)), logicalIds, 'V5 pending gate logical question set')

function validateResult(result, { schemaVersion, packageId, reviewer, submittedAt, expectedIds, label }) {
  ...
  assert(result.submitted_at === submittedAt, `${label} submitted_at mismatch`)
  assert(!Number.isNaN(Date.parse(result.submitted_at)), `${label} submitted_at must be a valid date-time`)
  ...
  for (const questionId of expectedIds) {
    const review = resultById.get(questionId)
    const candidate = candidatesById.get(questionId)
    assertExactKeys(review, ['question_id', 'previous_question_id', 'new_semantic_hash', 'candidate_record_hash', 'conclusion', 'review_note'], `${label}/${questionId}`)
    assert(/^M[1-6]_[A-Z]+_\d+_V5$/.test(review.question_id), `${questionId} invalid question_id`)
    assert(/^M[1-6]_[A-Z]+_\d+_V4$/.test(review.previous_question_id), `${questionId} invalid previous_question_id`)
    ...
    assert(['PASS', 'RETURN_FOR_REVISION'].includes(review.conclusion), `${questionId} invalid ${label} conclusion`)
...
const contentReview = validateResult(content, {
  schemaVersion: 'job-skill-shelver-298-targeted-v5-content-rereview-result-v1',
  packageId: 'job-skill-shelver-298-targeted-v5-content-rereview-packet-v1',
  reviewer: '陈晓青', submittedAt: '2026-07-20T10:06:56.006Z',
  expectedIds: expectedContentIds, label: 'content result'
})
const safetyReview = validateResult(safety, {
  schemaVersion: 'job-skill-shelver-298-targeted-v5-safety-technical-rereview-result-v1',
  packageId: 'job-skill-shelver-298-targeted-v5-safety-technical-rereview-packet-v1',
  reviewer: '赫东', submittedAt: '2026-07-20T10:07:50.985Z',
  expectedIds: expectedSafetyIds, label: 'safety/technical result'
})
...
const passedReferences = candidates.passed_question_references.map((reference) => {
  ...
  const base = {
    ...
    version_chain: {
      previous_question_id: source.previous_question_id ?? null,
      current_question_id: reference.question_id
    },
...
const mergedV5Questions = candidates.questions.map((candidate) => {
  const contentResult = contentReview.resultById.get(candidate.question_id)
  const safetyResult = safetyReview.resultById.get(candidate.question_id)
  const failedReviewTracks = [
    contentResult?.conclusion === 'RETURN_FOR_REVISION' ? 'CONTENT' : null,
    safetyResult?.conclusion === 'RETURN_FOR_REVISION' ? 'SAFETY_TECHNICAL' : null
  ].filter(Boolean)
  ...
  const base = {
    ...
    review_results: {
      content: contentResult
        ? { status: contentResult.conclusion, review_note: contentResult.review_note, source_record_hash: hashRecord(contentResult) }
        : { status: 'NOT_REQUIRED' },
      safety_technical: safetyResult
        ? { status: safetyResult.conclusion, review_note: safetyResult.review_note, source_record_hash: hashRecord(safetyResult) }
        : { status: 'NOT_REQUIRED' }
    },
    status,
    failed_review_tracks: failedReviewTracks,
    next_question_id: status === 'PASSED' ? null : candidate.question_id.replace(/_V5$/, '_V6')
  }
  return { ...base, merged_record_hash: hashRecord(base) }
})

const returned = mergedV5Questions.filter((question) => question.status === 'RETURN_FOR_REVISION')
const passedV5 = mergedV5Questions.filter((question) => question.status === 'PASSED')
const allQuestions = [...passedReferences, ...mergedV5Questions].sort((left, right) => stripVersion(left.question_id).localeCompare(stripVersion(right.question_id)))
assert(allQuestions.length === 145 && new Set(allQuestions.map((question) => stripVersion(question.question_id))).size === 145, 'Merged V5 gate must uniquely cover 145 questions')

const mergedBase = {
  schema_version: 'job-skill-shelver-298-targeted-v5-rereview-merged-gate-v1',
  gate_id: 'job-skill-shelver-298-targeted-v5-rereview-merged-gate-v1',
  status: returned.length === 0 ? 'PASSED' : 'COMPLETED_WITH_RETURNS',
  generated_at: '2026-07-20T18:15:00+08:00',
  ...
  source_results: {
    content: {
      source_path: 'E:\\Downloads\\job-skill-shelver-298-targeted-v5-content-rereview-packet-v1-result.json',
    ...
    safety_technical: {
      source_path: 'E:\\Downloads\\job-skill-shelver-298-targeted-v5-safety-technical-rereview-packet-v1-result.json',
  ...
  validation: {
    ...
    exact_content_question_set: expectedContentIds,
    exact_safety_technical_question_set: expectedSafetyIds,
    preserved_passed_reference_count: passedReferences.length,
    scope_anomalies: []
  },
  summary: {
    total: allQuestions.length,
    passed: passedReferences.length + passedV5.length,
    passed_references: passedReferences.length,
    passed_v5: passedV5.length,
...
const phaseConclusion = returned.length === 0
  ? '6道V5全部通过，阶段四可以作为下一步开始，但本轮未执行阶段四。'
  : `${returned.length}道仍需定点复修，阶段四继续关闭。`
const ledger = `# 超市理货员298题定点V5复审接收台账
...
|---|---|---|---:|---|
| 陈晓青：内容 | \`E:\\Downloads\\job-skill-shelver-298-targeted-v5-content-rereview-packet-v1-result.json\` | \`${rel(paths.content)}\` | ${statSync(paths.content).size} | \`${hashFile(paths.content)}\` |
| 赫东：安全与技术 | \`E:\\Downloads\\job-skill-shelver-298-targeted-v5-safety-technical-rereview-packet-v1-result.json\` | \`${rel(paths.safety)}\` | ${statSync(paths.safety).size} | \`${hashFile(paths.safety)}\` |
...
- 139道既有通过引用继续有效；6道V5中${passedV5.length}道通过，因此完整145题中共${merged.summary.passed}道通过。
- 仍退回${returned.length}道：${returnIds}。
- 只为实际退回题保留失败轨道和下一版题号；没有扩大复修或复审范围，也没有作废其他通过题。
- \`releaseable=${merged.authority.releaseable}\`、\`phase_4_allowed=${merged.authority.phase_4_allowed}\`；本轮未写运行库、未生成激活SQL，阶段二交付锁与所有历史题目保持不变。
`
...
console.log(`Targeted V5 review results ${checkOnly ? 'verified' : 'ingested'}: ${merged.summary.passed} passed, ${returned.length} returned; phase 4 ${merged.authority.phase_4_allowed ? 'may start next' : 'remains closed'}.`)
