...
const previousDeltaPaths = [2, 3, 4, 5, 6].map((step) => resolve(
  projectRoot,
  `scripts/fixtures/m5b-step${step}-source-delta-v1.json`
))
const deltaPath = resolve(projectRoot, 'scripts/fixtures/m5b-step7-source-delta-v1.json')
...
function targetClass(entry) {
  if (entry.file === 'src/main/application/services/student-service.ts' && entry.symbol === 'apply') {
    return 'GATE_ONLY_EXTERNAL_TRANSACTION_DML'
  }
  throw new Error(`[m5b-step7-fixture] unclassified added callsite ${entry.file}:${entry.symbol}`)
}
...
function updateSourceDelta() {
  ...
  for (let index = 0; index < deltas.length; index += 1) {
    ...
    if (deltas[index].step !== expectedStep || deltas[index].source_digest !== expectedSource) {
      throw new Error(`[m5b-step7-fixture] ${expectedStep} source chain is not accepted`)
    }
  }
  ...
  const exact = removed.length === 6
    && added.length === 6
    && removedCapabilities.length === 0
    && addedCapabilities.length === 0
    && [...removed, ...added].every((entry) => entry.file === 'src/main/application/services/student-service.ts')
    && added.every((entry) => entry.symbol === 'apply')
  ...
  if (!exact) throw new Error('[m5b-step7-fixture] checkout does not match the reviewed M5B-7 source delta')
  ...
  const fixture = {
    ...
    removed_source: removed.map((entry) => ({
      ...
      disposition: 'EXTRACTED_TO_GATE_ONLY_EXTERNAL_TRANSACTION_APPLY'
    })),
  ...
  if (existsSync(deltaPath)) {
    ...
    if (!exactJson(frozen, fixture)) {
      ...
      throw new Error('[m5b-step7-fixture] frozen source delta differs from checkout')
    }
...
console.log(`[m5b-step7-fixture] source_delta=${delta.status}`)
console.log(
  `[m5b-step7-fixture] digest=${delta.fixture.target_digest}, direct_removed=${delta.fixture.removed_source.length}, direct_added=${delta.fixture.added_target.length}`
)
