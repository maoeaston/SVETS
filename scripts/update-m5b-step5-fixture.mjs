...
const deltaPaths = [2, 3, 4].map((step) => resolve(
  projectRoot,
  `scripts/fixtures/m5b-step${step}-source-delta-v1.json`
))
const deltaPath = resolve(projectRoot, 'scripts/fixtures/m5b-step5-source-delta-v1.json')
...
function targetClass(entry) {
  if (entry.file === 'src/main/domain/event-batch/batch-coordinator.ts') {
    return 'EVENT_BATCH_COORDINATOR_INTERNAL'
  }
  if (entry.file === 'src/main/domain/event-batch/mixed-domain-replay.ts') {
    return 'EVENT_BATCH_REBUILD_INTERNAL'
  }
  if (entry.file === 'src/main/domain/event-batch/projection-source.ts') {
    return 'EVENT_BATCH_PROJECTION_INTERNAL'
  }
  if (entry.file === 'src/main/domain/event-batch/startup-recovery.ts') {
    return 'EVENT_BATCH_RECOVERY_INTERNAL'
  }
  throw new Error(`[m5b-step5-fixture] unclassified added callsite ${entry.file}:${entry.symbol}`)
}

function capabilityTargetClass(entry) {
  if (
    entry.file === 'src/main/domain/event-batch/mixed-domain-replay.ts'
    && entry.kind === 'LEGACY_RECOVERY_CALL'
    && entry.callee === 'reconcileActionLogEventGroup'
  ) {
    return 'EVENT_BATCH_REBUILD_LEGACY_COMPATIBILITY'
  }
  throw new Error(`[m5b-step5-fixture] unclassified added capability ${entry.file}:${entry.symbol}`)
}
...
function updateSourceDelta() {
  ...
  for (let index = 0; index < deltas.length; index += 1) {
    ...
    if (deltas[index].step !== expectedStep || deltas[index].source_digest !== expectedSource) {
      throw new Error(`[m5b-step5-fixture] ${expectedStep} source chain is not accepted`)
    }
  }
  ...
  const removedCapabilities = [...sourceCapabilities].filter(
    (fingerprint) => !targetCapabilities.has(fingerprint)
  )
  ...
  const exact = removed.length === 0
    ...
    && addedCapabilities.length === 1
    && added.every((entry) => expectedFiles.has(entry.file))
    && scan.channels.length === 74
    && scan.direct_callsites.length === 249
    && scan.direct_files.length === 36
    && scan.capability_callsites.length === 73
    && scan.delegating_roots.length === 13
  if (!exact) throw new Error('[m5b-step5-fixture] checkout does not match the reviewed M5B-5 source delta')
  ...
  const fixture = {
    ...
    capability_added_target: addedCapabilities.map((entry) => ({
      fingerprint: entry.fingerprint,
      file: entry.file,
      symbol: entry.symbol,
      line: entry.line,
      ...
      callee: entry.callee,
      occurrence: entry.occurrence,
      target_class: capabilityTargetClass(entry),
      planned_step: 'M5B-5'
    }))
  }
  if (existsSync(deltaPath)) {
    ...
    if (!exactJson(frozen, fixture)) {
      ...
      throw new Error('[m5b-step5-fixture] frozen source delta differs from checkout')
    }
...
console.log(`[m5b-step5-fixture] source_delta=${delta.status}`)
console.log(
  `[m5b-step5-fixture] digest=${delta.fixture.target_digest}, direct_added=${delta.fixture.added_target.length}, capability_added=${delta.fixture.capability_added_target.length}`
)
