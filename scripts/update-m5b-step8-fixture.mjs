...
const previousDeltaPaths = [2, 3, 4, 5, 6, 7].map((step) => resolve(
  projectRoot,
  `scripts/fixtures/m5b-step${step}-source-delta-v1.json`
))
const deltaPath = resolve(projectRoot, 'scripts/fixtures/m5b-step8-source-delta-v1.json')
...
function targetClass(entry) {
  if (entry.file === 'src/main/domain/projectors/training-projector.ts') {
    return 'TRAINING_PREPARED_PROJECTOR_INTERNAL'
  }
  throw new Error(`[m5b-step8-fixture] unclassified added callsite ${entry.file}:${entry.symbol}`)
}

export function verifyM5bStep8SourceDelta() {
  ...
  for (let index = 0; index < deltas.length; index += 1) {
    ...
    if (deltas[index].step !== expectedStep || deltas[index].source_digest !== expectedSource) {
      throw new Error(`[m5b-step8-fixture] ${expectedStep} source chain is not accepted`)
    }
  }
  ...
  const exact = removed.length === 0
    ...
    && added.every((entry) => entry.file === 'src/main/domain/projectors/training-projector.ts')
    && scan.channels.length === 74
    && scan.direct_callsites.length === 262
    && scan.direct_files.length === 38
    && scan.capability_callsites.length === 73
    && scan.delegating_roots.length === 13
  if (!exact) throw new Error('[m5b-step8-fixture] checkout does not match the reviewed M5B-8 source delta')
  ...
  if (existsSync(deltaPath)) {
    ...
    if (!exactJson(frozen, fixture)) {
      ...
      throw new Error('[m5b-step8-fixture] frozen source delta differs from checkout')
    }
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  const delta = verifyM5bStep8SourceDelta()
  console.log(`[m5b-step8-fixture] source_delta=${delta.status}`)
  console.log(
    `[m5b-step8-fixture] digest=${delta.fixture.target_digest}, direct_added=${delta.fixture.added_target.length}, capability_added=${delta.fixture.capability_added_target.length}`
  )
}
