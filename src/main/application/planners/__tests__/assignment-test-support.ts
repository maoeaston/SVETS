...
function suffix(slot: number): string {
  if (!Number.isSafeInteger(slot) || slot < 1 || slot > 999_999_999_999) throw new Error('invalid assignment test slot')
  return String(slot).padStart(12, '0')
}

function ids(slot: number) {
  return {
    commandId: `a1000000-0000-4000-8000-${suffix(slot)}`,
    batchId: `a2000000-0000-4000-8000-${suffix(slot)}`,
    clientId: `a3000000-0000-4000-8000-${suffix(slot)}`,
    idempotencyKey: `a4000000-0000-4000-8000-${suffix(slot)}`
  }
}

export interface AssignmentBatchHarness {
...
export async function createAssignmentBatchHarness(options: Readonly<{
  faultInjector?: EventBatchFaultInjector
  root?: string
}> = {}): Promise<AssignmentBatchHarness> {
  const ownsRoot = options.root === undefined
  const root = options.root ?? mkdtempSync(join(tmpdir(), 'svets-m5b-assignment-'))
  ...
  const coordinator = new EventBatchCoordinator({
    ...
    legacyAnchor: null,
    now: () => new Date(ASSIGNMENT_TEST_TIME),
...
export function acceptAssignmentCommand(options: Readonly<{
  harness: AssignmentBatchHarness
  slot: number
  commandType: string
  actor: Readonly<{ userId: string; role: 'TEACHER' | 'ADMIN' | 'STUDENT' }>
  ...
  const registered = options.harness.store.registerOrLoad({
    ...
    deviceId: 'assignment-device',
    authSessionId: `assignment-auth-${options.actor.userId}`,
    requestHash: String(options.slot % 10).repeat(64),
    eventBatchId: stableIds.batchId,
    createdAt: ASSIGNMENT_TEST_TIME,
  ...
  const row = options.harness.store.acquireLease({
    ...
    workerId: 'assignment-worker',
    now: ASSIGNMENT_TEST_TIME,
    allowFailed: true
  })
  if (!row) throw new Error('assignment command lease was not acquired')
