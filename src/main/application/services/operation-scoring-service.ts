// 实操评分（Operation Scoring）application service。
...
//
// submitOperationScores 接收 DBAdapter 与 accepted command execution。
...
export interface OperationScoringMutationExecution {
  readonly eventPort: Pick<ReportMutationPort, 'writeEvent'>
  readonly context: AcceptedCommandContext
}

function correlationFor(execution: OperationScoringMutationExecution): string {
  if (execution.context.envelope.commandType !== 'assessment:submitOperationScores') {
    throw new Error('operation scoring requires accepted assessment:submitOperationScores context')
  }
  const correlationId = execution.context.envelope.correlationId
  if (!correlationId.trim()) throw new Error('operation scoring correlation is required')
 ...
 *   → db.transaction { 9 × eventPort.writeEvent(OFFLINE_SCORE_SUBMITTED) + applyAssessmentEvent
 *                      + eventPort.writeEvent(RESULT_CALCULATED) + applyAssessmentEvent }
 */
export function submitOperationScores(
  db: DBAdapter,
  params: SubmitOperationScoresParams,
  execution: OperationScoringMutationExecution
): SubmitOperationScoresResult {
