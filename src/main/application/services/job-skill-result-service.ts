// T9: JOB_SKILL_SCORE 自动生成 application service。
...
const JOB_SKILL_RESULT_PARENTS = new Set([
  'assessment:submitJobSkillOfflineScores',
  'assessment:recordTeacherObservation'
])

export interface JobSkillResultMutationExecution {
  readonly eventPort: Pick<ReportMutationPort, 'writeEvent'>
  readonly context: AcceptedCommandContext
}

function correlationFor(execution: JobSkillResultMutationExecution): string {
  if (!JOB_SKILL_RESULT_PARENTS.has(execution.context.envelope.commandType)) {
    throw new Error(`job-skill result rejected parent ${execution.context.envelope.commandType}`)
  }
  const correlationId = execution.context.envelope.correlationId
  if (!correlationId.trim()) throw new Error('job-skill result correlation is required')
...
export function finalizeJobSkillResultCore(
  ...
  sessionId: string,
  callerUserId: string,
  execution: JobSkillResultMutationExecution
): boolean {
  ...
  const resultEvent = execution.eventPort.writeEvent({
...
export function maybeGenerateJobSkillReportAfterResult(
  ...
  correlationFor(execution)
  try {
    maybeGenerateJobSkillReport(db, sessionId, callerUserId, automation, execution.context)
  } catch (reportErr) {
...
export function maybeGenerateJobSkillResult(
  ...
  const txn = db.transaction(() => {
    generated = finalizeJobSkillResultCore(db, sessionId, callerUserId, execution)
  })
  txn()
  if (generated) {
    maybeGenerateJobSkillReportAfterResult(db, sessionId, callerUserId, automation, execution)
  }
}
