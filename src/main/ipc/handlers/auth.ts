...
export function getCurrentSession(db: DBAdapter, senderId: number): CurrentSessionResult {
  return resolveBoundAuthSession(db, senderId)
}
...
export function registerAuthHandlers(getDb: () => DBAdapter = defaultGetDb): void {
  ...
  ipcMain.handle('auth:login', (event, params: LoginParams) => {
    ...
    try {
      ...
      try {
        replaceSenderAuthSession(db, event.sender.id, result.rawToken)
      } catch (err) {
  ...
  ipcMain.handle('auth:getCurrentSession', (event) => {
    const db = ensureSeeded()

    try {
      return getCurrentSession(db, event.sender.id)
    } catch (err) {
  ...
  ipcMain.handle('auth:logout', (event) => {
    ...
    try {
      return logoutCurrentSession(db, event.sender.id)
    } catch (err) {
  ...
  ipcMain.handle('auth:listAccounts', (event, params: ListAccountsParams) => {
    const db = ensureSeeded()
    ...
    return listAccounts(db, trusted.params)
  })

  ipcMain.handle('auth:createTeacherAccount', (event, params: CreateTeacherAccountParams) => {
    const db = ensureSeeded()
    const trusted = resolveTrustedAuthSessionCaller(db, event.sender.id, params)
    if (!trusted.ok) {
      return { success: false as const, errorCode: 'FORBIDDEN' as const }
  ...
  ipcMain.handle('auth:setTeacherAccountStatus', (event, params: SetTeacherAccountStatusParams) => {
    const db = ensureSeeded()
    const trusted = resolveTrustedAuthSessionCaller(db, event.sender.id, params)
    if (!trusted.ok) {
      return { success: false as const, errorCode: 'FORBIDDEN' as const }
