...
| VID_A11 | M1_TF_025 | 把小件糖果集中放在中层显眼位置 | 正确 |
...
| VID_A16 | M3_TF_028 | 饼干有轻微褶皱封口完好将其上架 | 正确 |
...
| VID_B06 | M2_TF_022 | 徒手撕开胶带拆箱没有使用刀具商品完好取出 | 正确 |
...
| VID_C05 | M3_TF_030 | 用手机计算器计算保质期剩余天数 | 正确 |
| VID_C06 | M3_TF_031 | 已过期3天将其移入临期专区（应报损） | 错误 |
| VID_C07 | M3_TF_032 | 侧面凹陷但未漏气的可乐上架 | 错误 |
| VID_C08 | M3_TF_034 | 自行计算剩余9天直接移入临期专区 | 正确 |
...
| VID_D06 | M4_TF_019 | 自行发现数量差了1件重新清点标注 | 正确 |
...
│   工具: APIMart GPT-Image-2-official / 视频模型待定         │
└──────────────────────────┬──────────────────────────────┘
                           ▼
┌─────────────────────────────────────────────────────────┐
│ Layer 2: 程序合成层                                       │
