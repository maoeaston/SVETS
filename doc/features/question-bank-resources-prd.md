...
**合同版本：** `asset-manifest.schema.json v0.3.1`
...
1. Manifest 必须覆盖规划书明确列出的 231 个交付项和 6 个 R1-R6 参考资产。
...
8. `expected_answer` 必须与题库 `scoring_rule_json` 一致，题库始终是答案唯一事实来源。
9. AI 只能预审；视觉、职业、特教和测评门由人类签署。
10. 只有 `approved`、版权已确认、文件和 hash 一致、必需验收门全部通过的资产可以写入 `asset_resource` 为 `ACTIVE`。
11. 未出现在 approved 集合中的受管视觉资产必须降为 `DEPRECATED`。
...
1. `npm run asset:validate` 返回 `total=237` 且无错误。
