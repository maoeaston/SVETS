...
- Pilot 激活批准人由产品负责人通过安装版固定信任根和签名授权登记指定，且不得参与该版本内容制作、代码实现、门禁生成或激活执行；执行激活的 `ADMIN` 不能自授批准。
- 产品批准、学校课堂授权和逐学生同意都必须具有有限 `[effective_at, expires_at)`，状态由不可变签发件和追加式签名生命周期事件派生为 `PENDING / ACTIVE / EXPIRED / REVOKED / SUPERSEDED / INVALID`；产品根最长 365 天、批准人/学校授权人登记最长 90 天、责任清单/激活批准最长 30 天、学校课堂授权/逐学生同意最长 14 天，下游不得晚于上游。ADMIN 不能直接改状态、回填普通撤销或让终态授权复活。
- 学校机器可信链为“产品负责人根签名的学校授权人登记 → 登记学校责任人签名的课堂授权/逐学生同意”。同意绑定具体学生、模板和原始证据 hash；有权教师只能追加即时减权的撤回报告。产品索引最大租约 24 小时，存在课堂授权时学校/同意索引最大租约 4 小时；过期、回退/断链或授权时钟回拨时，真实 Pilot 运行失败关闭。
...
- 产品批准、学校授权或个人同意在开放 session 中失效时，系统停止输入，以授权失效原因关闭 sitting、事件化 `ABORTED` 并释放 grant/assignment；安全红线并发时 `REDLINE_HALTED` 优先。晚到失效与 session 运行区间重叠时，已完成事实保持不变，但通过追加事件永久隔离结果、报告和课堂证据；MVP 内事后重签或人工复核均不能恢复证据资格。
- 每个自然到期、撤销、替代、密钥泄漏或个人撤回都有稳定 `invalidation_fact_id`；多原因并存时逐项保留并取最早 `invalidation_at`，来源隔离按 session + fact ID 幂等。新增 sitting/assignment reason 或授权事件一旦写入历史，Schema 合同即为 expand-only：应用只能回滚到识别新旧值的兼容版本，不得降回旧 CHECK 或改写历史。
