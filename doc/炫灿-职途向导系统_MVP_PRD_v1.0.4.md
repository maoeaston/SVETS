...
版本：PRD v1.0.4-outline-and-consistency  
...
核心岗位样板：超市理货员  
MVP 核心任务：拆箱与上架  
文档状态：冻结候选版（v0.1.7 一致性保护与软件大纲边界已纳入；尚未进入正式编码落地）  
最后更新：2026-06-30
...
本 PRD 是《炫灿-职途向导系统 MVP》的冻结候选产品需求文档，已吸收以下决策与工程基线：
...
为避免大纲、PRD、schema 和组卷策略出现分叉，后续完整试卷系统采用以下题量口径：
...
- 线上题建议每个能力维度 3 道，即判断题、单选题、拖拽题各 1 道。
- 线下 5 道实操题必须通过复合能力标签覆盖 6 大基础能力，不要求机械做到每个维度 1 道。
...
教师端大纲中出现的“基础能力线上 18 题 + 线下 3 实操”和“专项综合线上 20 题 + 线下 4 实操”不作为最终口径，应在后续大纲修订中分别统一为“18 + 5”和“16 + 4”。
...
教师发起测评后，学生进入线上能力测评流程。测评用于判断学生是否理解“拆箱与上架”任务的基础规则。

#### 测评规则

测评题量、题型比例、满分、阈值由策略配置决定。

MVP 不允许在代码中硬编码题量和阈值。

MVP “拆箱与上架”任务测评的默认策略可采用：
...
后续“基础能力标准化模拟试卷”的固定规格另行定义为：

- 线上题 18 道。
- 线下实操 5 道。
- 覆盖 6 大基础能力维度。

两者不得混同。当前 MVP 任务测评以 `strategy_config` 为准；后续完整基础能力标准化模拟卷应进入 `schema v0.2.0-question-paper` 的正式试卷模型。
...
- `LEVEL_IMPROVE`：需巩固。
- `LEVEL_FAIL`：暂不达标。
- `LEVEL_FAIL_BY_SAFETY`：安全红线失败。

默认阈值由 `strategy_config` 控制。

MVP 初始建议：

- `normalized_score >= 70`：`LEVEL_PASS`
- `normalized_score >= 40 且 < 70`：`LEVEL_IMPROVE`
- `normalized_score < 40`：`LEVEL_FAIL`
- 安全红线触发：`LEVEL_FAIL_BY_SAFETY`
...
- 若 `safety_overridden = 1`，结论为“安全中止，暂不建议继续该任务实操”。
- 若 `OPERATION_PASS_RATE` 未达标，结论为“建议继续实操训练”。
- 若 `TRAINING_COMPLETION` 未完成，结论为“建议补完训练流程”。
- 若 `ABILITY_SCORE` 较低，结论为“建议回到规则理解训练”。
- 若三类结果均达标，结论为“当前任务阶段性达标”。
...
2. 已被 `assessment_session` 引用的 `strategy_config` 版本，修改 `pass_threshold` 应失败。
3. 已被 `assessment_session` 引用的 `strategy_config` 版本，修改 `question_policy_json` 应失败。
4. 已被 `training_session` 引用的 `strategy_config` 版本，修改 `scoring_policy_json` 应失败。
5. 已被任一 session 引用的 `strategy_config` 版本，修改 `strategy_name` 应失败。
6. 已被任一 session 引用的 `strategy_config` 版本，修改 `version` 应失败。
7. 已被引用的策略如需调整阈值或规则，必须插入新的 `version`。
8. 历史 session 通过 `strategy_id + strategy_version` 能够复现当时策略。
...
### 18.1 MVP v1.0.4-outline-and-consistency
...
- 完整试卷系统不得在 v1.0.4 / schema v0.1.7 中临时拼接实现，应进入 v1.1.0 与 schema v0.2.0。
